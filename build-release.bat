...
REM Copy mod files from src directory
echo Copying mod files...
xcopy /E /I /Q "src\%VERSION%" "%PACKAGE_DIR%\%VERSION%"
copy README.md "%PACKAGE_DIR%\"

REM Copy installation scripts
if exist "src\install.sh" copy "src\install.sh" "%PACKAGE_DIR%\"
if exist "src\install.bat" copy "src\install.bat" "%PACKAGE_DIR%\"

REM Copy build-tlk scripts (optional, for advanced users)
if exist "src\build-tlk.sh" copy "src\build-tlk.sh" "%PACKAGE_DIR%\"
if exist "src\build-tlk.bat" copy "src\build-tlk.bat" "%PACKAGE_DIR%\"

REM Check if TP2 file exists
if not exist "src\setup-%MOD_NAME%.tp2" (
    echo WARNING: setup-%MOD_NAME%.tp2 not found. You need to create it before release.
) else (
    copy "src\setup-%MOD_NAME%.tp2" "%PACKAGE_DIR%\"
)
